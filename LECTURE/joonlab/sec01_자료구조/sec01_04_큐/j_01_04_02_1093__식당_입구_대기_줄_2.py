...
# - 2: 식사 1인분이 준비된다.
def solution(n, A):
    # answer: [대기하는 학생 수의 최댓값, 이 때 맨 앞에 대기 중인 학생 번호]
    answer = [0, 0]
    ...
    for info in A:
        ...
        if info[0] == 1:
            ...
            q.append(info[1])
            # 큐의 맨 끝에 학생 번호를 삽입하고 정답을 갱신한다.
            # 식당 입구의 맨 앞에 대기 중인 학생 번호는 q[0]이다.
            if answer[0] < len(q) or (answer[0] == len(q) and answer[1] > q[0]):
                answer = [len(q), q[0]]
